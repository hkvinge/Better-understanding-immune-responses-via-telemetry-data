...
#Z =  1. - (x0[0]*X + x0[1]*Y)/( np.linalg.norm(x0)*np.sqrt(X**2 + Y**2) )
# Henry's "correlation distance" lies in [0,2]; make it [0,1] for comparison.
# Z *= 0.5

Z =  0.5*(1 + (x0[0]*X + x0[1]*Y)/( np.linalg.norm(x0)*np.sqrt(X**2 + Y**2) ))
...
fig.tight_layout()
fig.savefig('vector-cosine-vis-2d.png', dpi=120, bbox_inches='tight')
fig.savefig('vector-cosine-vis-2d.pdf', dpi=120, bbox_inches='tight')
